plot.developC <- function(xx = devel.df, post = TRUE,
                         rnd = 7, check.rounding = FALSE)
{
### Purpose:- Get rid of intercept
### ----------------------------------------------------------------------
### Modified from:- plot.developC2
### ----------------------------------------------------------------------
### Arguments:- rnd: how much to round coefficients
###             check.rounding: print out part of predictions
###                             from model and formula if req'd.
### ----------------------------------------------------------------------
### Author:-   Patrick Connolly, Date:- 29 Apr 2013, 16:08
### ----------------------------------------------------------------------
### Revisions:- 
...
options(digits = 10)
if(post){
    pdf(file = "Non-linearFitC.pdf", height = 195/25.4, width = 270/25.4)
...
xxF <- xx[xx$Sex == "F",]
browser()
rate.nlsM <- ## this one "works": don't erase (yet)
  nlxb(Dev ~ exp(tau * (T - Tmin))  -
       exp(rho*Tmax - (Tmax - T)/del), data = xxM, ## trace = TRUE, 
       lower = c(tau = .06, rho = .01, del = 1.2, Tmax = 29, Tmin = .004),
       upper = c(tau = 1.6, rho = 3, del = 9, Tmax = 35, Tmin = 7),
       start = c(tau = .3, rho = .8, del = 5, Tmax = 30, Tmin = 5))
...
rate.nlsF <- 
  nlxb(Dev ~ exp(tau * (T - Tmin)) -
       ...
       lower = c(tau = .06, rho = .01, del = .2, Tmax = 28, Tmin = .4),
       upper = c(tau = 1.6, rho = 3, del = 7, Tmax = 35, Tmin = 5),
       start = c(tau = .5, rho = .8, del = 5.5, Tmax = 30, Tmin = 3))
...
##     xlab = substitute(paste(Temperature, B * degree, "C)"), list(B = " ("))
     ...
     main = "Non-linear fit using Lactin modified Logan model with min Temp, without intercept",
...
formF <- substitute(D(T) == e^paste(b, (T-t)) - e^(d*T - z),
                    list(a = aF, b = bF, z = zF, d = dF, t = tF))
...
formM <- substitute(D(T) == e^paste(b, (T-t)) - e^(d*T - z),
                    list(a = aM, b = bM, z = zM, d = dM, t = tM))
...
dev.maxF <- with(coefF, exp(tau * (ToptF-Tmin)) -
              exp(rho*Tmax - (Tmax - ToptF)/del))
dev.maxM  <- with(coefM, exp(tau * (ToptM-Tmin)) -
              exp(rho*Tmax - (Tmax - ToptM)/del))
...
Ystart <- 0.052 # Begin table y value
Xstart <- 5
Xspace <- 1
tab.cex <- .9
tab.space <- 4 # width of table columns

text(Xstart + 4, Ystart + 1.5*ygap, "Model coefficients", cex = leg.cex)
text(Xstart + 3, Ystart - 8*ygap,
