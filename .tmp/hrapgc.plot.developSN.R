plot.developSN <- function(xx = devel14.df, post = TRUE,
           rnd = 7, check.rounding = FALSE, hilin = 24)
{
### Purpose:- Skewed-normal fit
### ----------------------------------------------------------------------
### Modified from:- plot.developC2_Na
### ----------------------------------------------------------------------
### Arguments:- hilin: highest temp in linear fit
### ----------------------------------------------------------------------
### Author:-   Patrick Connolly, Date:- 29 Jun 2015, 12:00
### ----------------------------------------------------------------------
### Revisions:- 
  ...
  options(digits = 10)
  if(post){
    pdf(file = paste0("Skewed-NormalFit.pdf"),
...
##   abline(v = rate.nlsF$coeff["Tmax"], col = colours[2], lty = 3)
  ...
  use.temp.range <- range(xx$TempRecord[xx$TempRecord < hilin])
