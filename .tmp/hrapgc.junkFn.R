junkFn <-
  function(xx = devel.df, post = FALSE, 
...
### Revisions:- 
  ...
  options(digits = 10)
  if(post){
    pdf(file = "Non-linearFitC2.pdf", height = 195/25.4, width = 270/25.4)
...
browser()
  ...
  plot(newdat$T, predM, type = "l", col = colours[1],
       ...
       main = "Non-linear fit using Lactin modified Logan model with min Temp, with intercept",
