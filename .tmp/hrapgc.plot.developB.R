plot.developB <- function(xx = devel.df, post = TRUE,
...
require(RColorBrewer)
  ...
  if(post){
    pdf(file = "Non-linearFitB.pdf", height = 195/25.4, width = 270/25.4)
...
##                 lam, data = xx, #
##                 trace = TRUE,
##                 start = c(rho = 0.1, del = 7.9, Tmax = 30, lam = .02))
 ##                exp(gam*Tmin), data = xx, #
 ##                 exp((T - Tmin)/tau), data = xx, #
     ##           exp(gam*Tmin - (T - Tmin)/tau) +
            ##      gam = .5,
           ##      tau = 10,
             ##     Tmin = 3))
