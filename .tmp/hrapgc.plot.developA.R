plot.developA <- function(xx = devel.df, post = TRUE,
                         rnd = 7, check.rounding = FALSE)
{
### Purpose:- Uses Lactin model unmodified (i.e. no lambda term)
### ----------------------------------------------------------------------
...
### ----------------------------------------------------------------------
### Author:-   Patrick Connolly, Date:- 19 Apr 2013, 11:17
### ----------------------------------------------------------------------
### Revisions:- 
...
require(RColorBrewer)
  ...
  if(post){
    pdf(file = "Non-linearFitA.pdf", height = 195/25.4, width = 270/25.4)
...
rate.nlsF2 <- nls(Dev ~ exp(rho * T) - exp(rho*Tmax - (Tmax - T)/del),
                 data = xx, subset = Sex == "F", #trace = TRUE,
                  #control = nls.control(maxiter = 100),
                  start = c(rho = 0.17, del = 5.6, Tmax = 31))
coefF <- as.list(coef(rate.nlsF2))
coefM <- as.list(coef(rate.nlsM2))
...
predM <- predict(rate.nlsM2, newdat)
predF <- predict(rate.nlsF2, newdat) # no lambda (I think)
...
plot(newdat$T, predM, type = "l", col = colours[1],
     ...
     main = "Non-linear fit using original Logan model",
     ylab = "Development rate (/day)", cex.lab = 1.3)
abline(v = coef(rate.nlsM2)["Tmax"], col = colours[1], lty = 3)
abline(v = coef(rate.nlsF2)["Tmax"], col = colours[2], lty = 3)
abline(h = 0, col = "grey")
...
formF <- substitute(D(T) == e^paste(a, T) - e^(d*T - z) ,
...
formM <- substitute(D(T) == e^paste(a ,T) - e^(d*T - z),
...
derivF <- with(coefF, deriv(~exp(rho*T) - exp(rho*Tmax - (Tmax - T)/del) ,
                            "T"))
derivM <- with(coefM, deriv(~exp(rho*T) - exp(rho*Tmax - (Tmax - T)/del),
                            "T")) 
...
dev.maxF <- exp(aF * ToptF) - exp(dF * ToptF - zF)  # max dev rates
dev.maxM <- exp(aM * ToptM) - exp(dM * ToptM - zM) 
...
text(FassyT + xgap/10, -0.0007, round(FassyT, 2), cex = temp.cex,
     col = colours[2], adj = 0)
text(MassyT - xgap/10, -0.0007, round(MassyT, 2), cex = temp.cex,
     col = colours[1], adj = 1)
...
segments(Xstart + Xspace/2, Ystart+ ygap/2,  Xstart + Xspace/2,
         Ystart - 3.5*ygap, lwd = .5)
segments(Xstart + Xspace/2+ tab.space, Ystart+ ygap/2,
         Xstart + Xspace/2+ tab.space, Ystart - 3.5*ygap, lwd = .5)
